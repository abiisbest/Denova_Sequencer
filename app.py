import streamlit as st
import gzip
import re
import pandas as pd
import plotly.graph_objects as go
import numpy as np
import io

st.set_page_config(page_title="De Novo Professional Suite", layout="wide")

SPECIES_LIBRARY = {
    "Escherichia coli (K-12)": {"ref_gc": 50.8, "expected_genes": 4300, "genome_size": 4641652},
    "Staphylococcus aureus": {"ref_gc": 32.8, "expected_genes": 2800, "genome_size": 2821361},
    "Bacillus subtilis": {"ref_gc": 43.5, "expected_genes": 4200, "genome_size": 4214630},
    "Saccharomyces cerevisiae (Yeast)": {"ref_gc": 38.3, "expected_genes": 6000, "genome_size": 12157105},
    "Mycobacterium tuberculosis": {"ref_gc": 65.6, "expected_genes": 4000, "genome_size": 4411532}
}

st.title("🧬 De Novo: Auto-Identifying Genomic Pipeline")
st.markdown("---")

def format_indian_num(n):
    if n >= 100000:
        return f"{n/100000:.2f}L"
    return f"{n:,}"

def get_rev_complement(seq):
    complement = {'A': 'T', 'C': 'G', 'G': 'C', 'T': 'A', 'N': 'N'}
    return "".join(complement.get(base, base) for base in reversed(seq))

def find_all_orfs(sequence, min_len=90):
    found_genes = []
    pattern = re.compile(r'(ATG(?:...){%d,2000}?(?:TAG|TAA|TGA))' % (min_len // 3))
    for strand in ["Forward", "Reverse"]:
        dna = sequence if strand == "Forward" else get_rev_complement(sequence)
        for frame in range(3):
            for match in pattern.finditer(dna[frame:]):
                gene_seq = match.group()
                start_pos = match.start() + frame
                found_genes.append({
                    "Strand": strand, "Start": int(start_pos), "End": int(start_pos + len(gene_seq)),
                    "Length": int(len(gene_seq)), "GC %": round((gene_seq.count('G') + gene_seq.count('C')) / len(gene_seq) * 100, 2),
                    "Sequence": gene_seq, "Type": "Gene"
                })
    return found_genes

def parse_genomic_file(uploaded_file):
    name = uploaded_file.name.lower()
    if name.endswith('.gz'):
        content = gzip.decompress(uploaded_file.read()).decode("utf-8")
    else:
        content = uploaded_file.read().decode("utf-8")
    
    lines = content.splitlines()
    sequences = []

    if any(name.endswith(ext) for ext in [".fasta", ".fa", ".fna"]):
        current_seq = []
        for line in lines:
            if line.startswith(">"):
                if current_seq: sequences.append("".join(current_seq))
                current_seq = []
            else:
                current_seq.append(line.strip())
        if current_seq: sequences.append("".join(current_seq))
    elif any(name.endswith(ext) for ext in [".fastq", ".fq"]):
        sequences = [line.strip() for line in lines[1::4] if line.strip()]
    else:
        sequences = [line.strip() for line in lines if line.strip() and not line.startswith(('@', '>', '+'))]
    return sequences

uploaded_file = st.file_uploader("Upload Genomic Data (FASTA/FASTQ/GZ)", type=["fastq", "fq", "fasta", "fa", "fna", "gz"])

if uploaded_file:
    try:
        raw_reads = parse_genomic_file(uploaded_file)
        if not raw_reads:
            st.error("No valid sequences detected.")
            st.stop()

        sample_seq = "".join(raw_reads[:100])
        sample_gc = round((sample_seq.count('G') + sample_seq.count('C')) / len(sample_seq) * 100, 2)
        closest_species = min(SPECIES_LIBRARY.keys(), key=lambda x: abs(SPECIES_LIBRARY[x]['ref_gc'] - sample_gc))
        
        st.subheader("🤖 Auto-Identification Result")
        st.info(f"Sample GC Content: **{sample_gc}%** | Predicted Species: **{closest_species}**")
        
        is_correct = st.radio("Confirm Species Identity:", ("Yes, proceed", "No, manual selection"))
        selected_species = closest_species if "Yes" in is_correct else st.selectbox("Select Species", list(SPECIES_LIBRARY.keys()))
        ref = SPECIES_LIBRARY.get(selected_species)

        if st.button("🚀 Run Full Analysis"):
            is_fastq = any(uploaded_file.name.lower().endswith(ext) for ext in [".fastq", ".fq"])
            processed_reads = [r[5:-5] for r in raw_reads if len(r) > 100] if is_fastq else raw_reads
            
            full_genome = "".join(processed_reads[:100])
            total_len = len(full_genome)
            
            all_orfs = find_all_orfs(full_genome)
            genes_df = pd.DataFrame(all_orfs).sort_values('Start').drop_duplicates(subset=['Start', 'Strand'])
            
            tab1, tab2, tab3 = st.tabs(["📊 QC Report", "🏗️ Alignment", "🧬 Annotation"])

            with tab1:
                col1, col2 = st.columns(2)
                col1.metric("Total Sequences", format_indian_num(len(raw_reads)))
                col2.metric("Analyzed Length", f"{total_len:,} bp")
                st.table(pd.DataFrame({
                    "Metric": ["GC Content", "ORFs Found", "Coding Density"],
                    "Value": [f"{sample_gc}%", len(genes_df), f"{round((genes_df['Length'].sum()/total_len)*100, 2) if total_len > 0 else 0}%"]
                }))

            with tab2:
                window = max(100, total_len // 50)
                p_skew, skews = [], []
                for i in range(0, total_len - window, window):
                    sub = full_genome[i:i+window]
                    g, c = sub.count('G'), sub.count('C')
                    skews.append((g - c) / (g + c) if (g + c) > 0 else 0)
                    p_skew.append(i)
                
                fig_skew = go.Figure()
                fig_skew.add_trace(go.Scatter(x=p_skew, y=skews, fill='tozeroy', line=dict(color='#00CC96')))
                fig_skew.update_layout(title="GC Skew Analysis", template="plotly_dark")
                st.plotly_chart(fig_skew, use_container_width=True)

            with tab3:
                st.subheader("🧬 Refined ORF Mapping")
                fig_map = go.Figure()
                for strand in ["Forward", "Reverse"]:
                    sdf = genes_df[genes_df["Strand"] == strand].sort_values('Start')
                    last_end, plot_data = 0, []
                    for _, row in sdf.iterrows():
                        if row['Start'] > last_end:
                            plot_data.append({"Start": last_end, "Len": row['Start'] - last_end, "Type": "Non-Coding"})
                        
                        actual_start = max(row['Start'], last_end)
                        actual_len = row['End'] - actual_start
                        if actual_len > 0:
                            plot_data.append({"Start": actual_start, "Len": actual_len, "Type": "Gene"})
                        last_end = max(last_end, row['End'])
                    
                    pdf = pd.DataFrame(plot_data)
                    if not pdf.empty:
                        colors = ['#1A1A1A' if t == "Non-Coding" else '#00CC96' for t in pdf['Type']]
                        fig_map.add_trace(go.Bar(x=pdf["Len"], y=[strand]*len(pdf), base=pdf["Start"], orientation='h', marker=dict(color=colors, line=dict(width=0))))
                
                fig_map.update_layout(barmode='stack', template="plotly_dark", showlegend=False, height=400)
                st.plotly_chart(fig_map, use_container_width=True)
                st.dataframe(genes_df.drop(columns=['Sequence']))

                st.subheader("📂 Export")
                st.download_button("📄 Download Results (CSV)", genes_df.to_csv(index=False), "genomic_results.csv")

    except Exception as e:
        st.error(f"Analysis Error: {e}")
